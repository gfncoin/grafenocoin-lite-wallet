...
int main(int argc, char **argv)
{
    ...
    /* Our connection to GrafenoCoind */
         ...
         Config &config)
{
    ...
    do
    {
        std::cout << InformationMsg("GrafenoCoin v"
    ...
    {
        std::cout << WarningMsg("It looks like GrafenoCoind isn't open!")
                  << std::endl << std::endl
                  << WarningMsg("Ensure GrafenoCoind is open and has finished "
                                "initializing.")
                  << std::endl
                  << WarningMsg("If it's still not working, try restarting "
                                "GrafenoCoind. The daemon sometimes gets stuck.") 
                  << std::endl
                  << WarningMsg("Alternatively, perhaps GrafenoCoind can't "
...
std::shared_ptr<WalletInfo> createViewWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    while (true)
    {
        ...
        else if (address.substr(0, 4) != "GFN")
        {
            std::cout << WarningMsg("Invalid address! It should start with "
                                    "\"GFN\"!") << std::endl;
                                                        ...
                                                        address))
        {
            std::cout << WarningMsg("Failed to parse GFN address! Ensure you "
...
void help(bool viewWallet)
{
    std::cout << "Available commands:" << std::endl
              ...
              << "Display how much GFN you have" << std::endl
    ...
    if (viewWallet)
    {
        std::cout << InformationMsg("Please note you are using a view only "
                                    "wallet, and so cannot transfer GFN.")
                  << std::endl;
    }
    else
    {
                           ...
                           CryptoNote::INode &node)
{
    ...
    /* Couldn't get timestamp, maybe old node or GrafenoCoind closed */
    if (blockTime != "")
    {
                         ...
                         std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    if (localHeight != remoteHeight)
    {
        std::cout << "Your GrafenoCoind isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting GrafenoCoind.\n"
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[GFN " + shortName + "]: ");
}

void connectingMsg()
{
    std::cout << std::endl << "Making initial contact with GrafenoCoind."
              << std::endl
...
void viewWalletMsg()
{
    std::cout << InformationMsg("Please remember that when using a view wallet "
              ...
              << std::endl << "This means if you received 100 GFN and then "
              << "sent 50 GFN, your balance would appear to still be 100 "
              << "GFN." << std::endl
