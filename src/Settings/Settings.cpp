...
const char DEFAULT_WALLET_FILE_NAME[] = "grafenocoinwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "grafenocoin.work:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("grafenocoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("grafenocoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("grafenocoinwallet.desktop");
  ...
  res = autorunSettings.contains("BytecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("GrafenoCoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.grafenocoin.grafenocoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=GrafenoCoin Lite\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("GrafenoCoinWallet", appPath);
    } else {
      autorunSettings.remove("GrafenoCoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\grafenocoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:grafenocoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\grafenocoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\grafenocoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("grafenocoinwallet.cfg"));
